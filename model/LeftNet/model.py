...
class NeighborEmb(MessagePassing):
    def __init__(self, hid_dim: int):
        ...
        self.embedding = nn.Linear(524, hid_dim)
...
class LEFTNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.z_linear = nn.Linear(524, hidden_channels)  # 新增线性层
        ...
        self.last_layer = nn.Linear(hidden_channels, 524)
    ...
    def forward(self, batch_data):
        z, pos, batch = batch_data['x'], batch_data['coords_ca'], batch_data['batch']

        if self.pos_require_grad:
            pos.requires_grad_()

        # # 进行特征标准化
        # z = self.normalize_features(z)
