...
def indexFromCoord(coord, coord_list):
    # 从 coord_list 中查找某个坐标 coord 的索引
    try:
...
def dist_list(point, coord_list):
    # 计算一个点 point 与一组点 coord_list 中每个点之间的距离
    dist_list = []
...
def min_max(index, point, coord_list):
    # 计算点 point 与坐标列表中的其他点之间的距离特征，包括最小、最大距离以及其他几何关系
...
def tri_location(path_name, out_path=None, save_npz=False):  # 输入
    # 读取文件中的原子坐标，计算每个原子的几何特征，并返回或保存为 .npz 文件
...
def tri_location_D(cb_map, cb_coords):  # input is dist map
    # 基于距离矩阵 cb_map 和坐标列表 cb_coords，计算每个残基的几何特征
    nres = len(cb_map)
