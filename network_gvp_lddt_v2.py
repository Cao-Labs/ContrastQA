...
class QAModel(pl.LightningModule):
    def __init__(self):
        ...
        self.W_v = nn.Sequential(
            ...
            GVP((self.node_scalar_feature_dim, 0),
                (self.node_scalar_feature_dim, self.node_vector_feature_dim),
                activations=(None, None), vector_gate=True)
        )
    ...
    def forward(self, g, h, x, edge_index, edge_attr, scatter_mean=True, dense=True):
        ...
        # 添加残差连接
        tensor1, tensor2 = node_inputs
        x = x.unsqueeze(-1)

        tensor2 = tensor2 + x  # 将 x 加到 tensor2 上

        node_inputs = (tensor1, tensor2)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # 模型前向传播
        pred_lddt = self.forward(dgl_graph, batch_n, coords, edge_index, edge_attr, scatter_mean=True, dense=True)
        # 获取当前批次大小
        batch_size = dgl_graph.batch_size

        # lddt loss
        lddt_loss = F.mse_loss(pred_lddt.squeeze(), batch_targets.float())
        # print(f"train_loss: {lddt_loss}")
