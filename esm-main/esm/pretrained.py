...
def load_model_and_alphabet_local(model_location):
    ...
    # model_data = torch.load(str(model_location))
...
def load_model_and_alphabet_core(model_name, model_data, regression_data=None):
    ...
    if regression_data is None:
        ...
        if error_msgs:
            raise RuntimeError(
                r"Error(s) in loading state_dict for {}:\n\t{}".format(
...
def esm_if1_gvp4_t16_142M_UR50():
    ...
    Returns a tuple of (Model, Alphabet).
    """
    pt_dir=os.path.abspath(os.path.dirname(__file__))+"-pt"
    if_pt=os.path.join(pt_dir, "esm_if1_gvp4_t16_142M_UR50.pt")
    return load_model_and_alphabet_local(if_pt)
...
def esm2_t33_650M_UR50D():
    ...
    """
    pt_dir=os.path.abspath(os.path.dirname(__file__))+"-pt"
    esm2_pt=os.path.join(pt_dir, "esm2_t33_650M_UR50D.pt")
    return load_model_and_alphabet_hub(esm2_pt)
...
def esmfold_v0():
    ...
    """
    import esm_main.esm.esmfold.v1.pretrained
